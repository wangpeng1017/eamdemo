...
> ```
>
> **根本原因**：`.next/static/` 目录容易在部署过程中丢失，导致浏览器无法加载 chunk 文件。
>
> **永久解决**：每次部署必须确保3个文件都上传：
> 1. ✅ standalone.tar.gz（服务器代码）
> 2. ✅ static.tar.gz（客户端chunk文件） ← **最容易遗漏！**
> 3. ✅ public.tar.gz（静态资源）
