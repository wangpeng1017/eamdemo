...
### 7.8 ChunkLoadError: Loading chunk failed (404) â ï¸ å¸¸è§é®é¢

**é®é¢**ï¼é¡µé¢å è½½æ¥é `ChunkLoadError: Loading chunk xxx failed`,æµè§å¨æ§å¶å°æ¾ç¤º `/_next/static/chunks/app/(dashboard)/xxx/page-xxx.js` 404

**æ ¹æ¬åå **ï¼é¨ç½²æ¶**éæ¼äº `.next/static` ç®å½**

Next.js standalone æ¨¡å¼çç®å½ç»æï¼
- `.next/standalone/.next/` - åªåå«æå¡å¨ç«¯ä»£ç ï¼æ  staticï¼
- `.next/static/` - å®¢æ·ç«¯ JS/CSS chunk æä»¶ï¼éè¦åç¬æåä¸ä¼ ï¼ï¼
- `public/` - å¬å±éæèµæº

**å®æ´è§£å³æ¹æ¡ï¼æå¯é ï¼**ï¼

```bash
# === æ¹æ³1ï¼ä»æ¬å°éæ°ä¸ä¼  staticï¼æ¨èï¼===

# 1. æ¬å°æå static ç®å½
cd /path/to/local/project/.next
tar -czf static.tar.gz static

# 2. ä¸ä¼ å°æå¡å¨
sshpass -p 'xx198910170014Z' scp -o ServerAliveInterval=10   static.tar.gz root@8.130.182.148:/root/lims-next/

# 3. æå¡å¨ç«¯è§£å
ssh root@8.130.182.148 "
  cd /root/lims-next &&   tar -xzf static.tar.gz -C .next/ &&   rm -f static.tar.gz &&   pm2 restart lims-next
"

# 4. éªè¯ä¿®å¤ï¼åºè¿å 200ï¼
curl -I http://8.130.182.148:3001/_next/static/chunks/framework-xxx.js
```

**é¢é²æªæ½**ï¼æ´æ° `deploy-fast.sh` èæ¬ï¼ç¡®ä¿æå staticï¼

```bash
# å¨æåæ­¥éª¤æ·»å  static
cd .next && tar -czf standalone.tar.gz standalone && cd ..
cd .next && tar -czf static.tar.gz static && cd ..  # â å¿é¡»æ·»å è¿è¡
tar -czf public.tar.gz public

# ä¸ä¼ æ¶ä¹è¦ä¸ä¼  static.tar.gz
sshpass -p "$SERVER_PASS" scp .next/static.tar.gz "$SERVER:$REMOTE_DIR/"
```

**éªè¯æ¸å**ï¼
- [ ] `.next/static/chunks/` ç®å½å­å¨
- [ ] chunk æä»¶æ°é > 0ï¼`find .next/static -name "*.js" | wc -l`
- [ ] æµè§å¨è®¿é®ä»»æ chunk æä»¶è¿å 200
- [ ] é¡µé¢å·æ°åæ  ChunkLoadError
### 7.9 生产环境缺失 .next 目录
