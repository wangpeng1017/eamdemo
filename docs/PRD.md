...
- ✅ **移除独立的部门管理菜单**：
  - 原因：功能与用户管理中的组织架构重复
  - 优化：部门管理功能完全集成到 `/system/user` 页面的左侧组织架构树
  - 影响：减少菜单冗余，提升用户体验
  - 保留：原 `/system/dept` 页面代码和 `/api/dept` 接口继续提供服务
- ✅ **更新 PRD 文档**：说明部门管理的访问方式和功能集成情况

**审批流程增强**：
- ✅ **审批节点支持下拉选择**：
  - 角色类型：从角色列表中选择（显示角色名称）
  - 指定用户：从用户列表中选择（显示：姓名 (联系方式)）
  - 部门负责人：从部门列表中选择（显示：部门名称）
  - 自动填充显示名称，可手动修改
  - 切换类型时智能清空已选择的内容

**角色管理优化**：
- ✅ **创建"普通用户"角色**：替代原有业务角色（销售经理、销售人员、实验室负责人、检测人员、财务人员、样品管理员）
- ✅ **迁移用户数据**：7 个用户从业务角色迁移到"普通用户"角色
- ✅ **业务角色可删除**：所有业务角色（finance, lab_director, sales_manager, sample_admin, tester）用户数为 0，可安全删除
- 迁移用户列表：财务负责人、实验室主任、销售负责人、张馨、姜艺玺、王璐、检测员
