...
### 文档
- **单据状态流转分析**
  - 新增 `docs/STATUS_FLOW_ANALYSIS.md` - 全系统单据状态机梳理
  - 识别 5 个关键问题：
    - 委托单状态定义与实际运行不一致（Schema 定义 4 个，实际用 3 个）
    - 委托单缺少"报告已生成"状态（核心业务缺口）
    - 检测任务状态混用中英文
    - 报价单 StatusTag 存在死状态 `entrusted`
    - 审批状态字段冗余（`status` 与 `approvalStatus`）
  - 提出两种修复方案：
    - 方案A：扩展委托单主状态机（新增 `report_issued`）
    - 方案B：增加独立 `reportStatus` 字段（轻量）
