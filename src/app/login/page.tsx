...
export default function LoginPage() {
 ...
 const handleLogin = async (phone: string, password: string) => {
  console.log('[登录] 开始登录:', { phone })
 setLoading(true)
  try {
 console.log('[登录] 调用signIn...')
 ...
 password,
  ...
  console.log('[登录] signIn结果:', result)
 ...
 if (result?.error) {
  ...
  console.error('[登录] 捕获异常:', error)
  showError(`登录异常: ${error instanceof Error ? error.message : String(error)}`)
 } finally {
 setLoading(false)
