    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (items.length > 0) {
        const createData = items.map((item: any, index: number) => ({
          ...
          testMethod: item.testMethod?.trim() || null,
          samplingLocation: item.samplingLocation?.trim() || null,
          specimenCount: item.specimenCount?.trim() || null,
          testRemark: item.testRemark?.trim() || null,
          // 材料级独有字段
          materialName: item.materialName?.trim() || null,
          materialCode: item.materialCode?.trim() || null,
          materialSupplier: item.materialSupplier?.trim() || null,
          materialSpec: item.materialSpec?.trim() || null,
          materialSampleStatus: item.materialSampleStatus?.trim() || null,
          // 评估相关字段
