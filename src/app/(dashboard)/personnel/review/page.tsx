...
export default function CapabilityReviewPage() {
  ...
  const handleDelete = async (id: string) => {
    const res = await fetch(`/api/capability-review/${id}`, { method: 'DELETE' })
    const json = await res.json()
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small" style={{ whiteSpace: 'nowrap' }}>
          ...
          <Popconfirm title="确认删除该评审记录？" onConfirm={() => handleDelete(record.id)} okText="确定" cancelText="取消" okButtonProps={{ danger: true }}>
