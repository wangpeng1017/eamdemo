...
export default function EditDevicePageClient({ id }: EditPageClientProps) {
    const router = useRouter()
  const goBack = useGoBack('/device/list')
    ...
    const fetchData = async () => {
        try {
            ...
            if (json.success) {
                form.setFieldsValue({
                    ...json.data,
                    calibrationDate: json.data.calibrationDate ? dayjs(json.data.calibrationDate) : null,
                    nextCalibration: json.data.nextCalibration ? dayjs(json.data.nextCalibration) : null,
                })
            } else {
                showError('加载数据失败')
            }
    ...
    return (
        <div style={{ padding: '0 24px 24px', minHeight: '100vh', background: '#f0f2f5' }}>
            <div style={{ marginBottom: 16, paddingTop: 16 }}>
                ...
                <span style={{ fontSize: 20, fontWeight: 500, marginLeft: 8 }}>编辑设备</span>
            </div>
